...
__local__ int output_local[1024];
...
void main(tensor input, tensor output, unsigned int partitions, unsigned int partition_size, unsigned int bins)
{
    ...
    for(int i = start[0]; i < end[0]; i++)
    {
        ...
        for(int j = 0; j < partition_size; j+=step)
        {
            ...
            output_local[target_coord_0[1]] = s_i32_add(output_local[target_coord_0[1]], one, 0, 0, s_cmp_res_0, 0);
            output_local[target_coord_1[1]] = s_i32_add(output_local[target_coord_1[1]], one, 0, 0, s_cmp_res_1, 0);
            output_local[target_coord_2[1]] = s_i32_add(output_local[target_coord_2[1]], one, 0, 0, s_cmp_res_2, 0);
            output_local[target_coord_3[1]] = s_i32_add(output_local[target_coord_3[1]], one, 0, 0, s_cmp_res_3, 0);
